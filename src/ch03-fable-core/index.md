...
이 장은 Fable.Core의 입문 범위만 다룹니다. 실전 라이브러리 바인딩을 위한 `[<Emit>]`과 dynamic `?` 연산자는 4장에서, erased union(`U2`–`U9`)과 `[<StringEnum>]`은 5장에서, `[<JS.Pojo>]`와 POJO 패턴은 6장에서 다룹니다. 이 개념들의 이름은 이미 언급했지만, 실제 사용법과 예제는 해당 장에서 제공합니다.
...
- 이 장에서 다룬 것은 입문 범위입니다. 실전 `[<Emit>]` 바인딩과 dynamic `?`(4장), erased union과 `[<StringEnum>]`(5장), POJO 패턴(6장)은 이후 장에서 다룹니다.
